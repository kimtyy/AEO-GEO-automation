...
답변 작성 시 다음 원칙을 반드시 지켜주세요:
1. 모호한 표현('좋아요', '맛있어요', '추천해요') 대신 구체적인 사실(주소, 영업시간, 좌석 수, 메뉴명, 가격대 등)을 명시할 것
2. 답변만 읽어도 그 자체로 완결된 정보가 되도록 작성할 것 (앞뒤 맥락 없이도 이해 가능해야 함)
3. 신뢰를 줄 수 있는 구체적 근거(경력, 인증, 특허, 레시피 출처 등)가 매장 정보에 있다면 반드시 포함할 것
4. 한 답변에 한 가지 핵심 정보만 담아 간결하게 작성할 것 (2~3문장 이내)
5. 광고성 수식어(최고의, 환상적인 등)는 배제하고 사실 위주로 작성할 것
